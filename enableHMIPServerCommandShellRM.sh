...
if [[ $ARCH == "x86_64" ]]; then 
  ...
  echo -n "libxrender-0.9.10-5 - " ; wget -qO- http://mirror.archlinuxarm.org/aarch64/extra/libxrender-0.9.10-5-aarch64.pkg.tar.xz | tar xJf - -C / ; if [ "$?" -eq 0 ]; then echo "OK"; else echo "ERROR"; fi
  echo -n "libxtst-1.2.3-5 - "     ; wget -qO- http://mirror.archlinuxarm.org/aarch64/extra/libxtst-1.2.3-5-aarch64.pkg.tar.xz     | tar xJf - -C / ; if [ "$?" -eq 0 ]; then echo "OK"; else echo "ERROR"; fi
  ...
  echo -n "libxau-1.0.9-4 - "      ; wget -qO- http://mirror.archlinuxarm.org/aarch64/extra/libxau-1.0.9-4-aarch64.pkg.tar.xz      | tar xJf - -C / ; if [ "$?" -eq 0 ]; then echo "OK"; else echo "ERROR"; fi
  echo -n "libxmu-1.1.3-3 - "      ; wget -qO- http://mirror.archlinuxarm.org/aarch64/extra/libxmu-1.1.3-3-aarch64.pkg.tar.xz      | tar xJf - -C / ; if [ "$?" -eq 0 ]; then echo "OK"; else echo "ERROR"; fi
  ...
  echo -n "libbsd-0.11.6-2 - "     ; wget -qO- http://mirror.archlinuxarm.org/aarch64/extra/libbsd-0.11.6-2-aarch64.pkg.tar.xz     | tar xJf - -C / ; if [ "$?" -eq 0 ]; then echo "OK"; else echo "ERROR"; fi
